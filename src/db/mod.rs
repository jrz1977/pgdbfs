...
impl PgDbMgr {
    ...
    pub fn lookup(&mut self, mnt_pt: &String, ino: i64, name: &str) -> Option<Ent> {
        println!(
            "** {} - lookup called for: mnt_pt: {}, ino: {}, name: {}",
            TAG, mnt_pt, ino, name
        );
        let conn = self.connect();
        for row in &conn.query("select id, ino, name, is_dir, size, create_ts, update_ts from pgdbfs where mnt_pt=$1 and parentid=$2 and name=$3",
    ...
    pub fn lookup_by_ino(&mut self, mnt_pt: &String, ino: i64) -> Option<Ent> {
        ...
        let conn = self.connect();
        for row in &conn.query("select id, ino, name, is_dir, size, create_ts, update_ts from pgdbfs where mnt_pt=$1 and ino=$2",
                               &[&mnt_pt, &ino]).unwrap() {
            let e = Ent {
                id: row.get(0),
                ino: row.get(1),
    ...
    pub fn read(&mut self, mnt_pt: &String, ino: i64, offset: i64, size: u32) -> Option<EntData> {
        let conn = self.connect();
        let sql = "select ino, data from pgdbfs where mnt_pt=$1 and ino=$2";
        for row in &conn.query(sql, &[&mnt_pt, &ino]).unwrap() {
    ...
    pub fn write(&mut self, mnt_pt: &String, ino: i64, offset: i64, data: &[u8]) -> u64 {
        let conn = self.connect();
        ...
        match self.lookup_by_ino(mnt_pt, ino) {
            ...
            Some(ent) => {
                let sql = "insert into pgdbfs_data (id, fsid, file_offset, data) values
                           ( (select nextval('fsid_seq')), $1, $2, $3)";
                println!("Inserting data for fsid: {}, offset: {}", ent.id, offset);
                match conn.execute(sql, &[&ent.id, &offset, &data]) {
                    Result::Ok(val) => {
                        return val;
                    }
                    Result::Err(err) => {
